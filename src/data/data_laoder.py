...
class MMSDataset(Dataset):
        ...
        for _id in self.ids:
            ...
            if self.use_ig65m:
                _video_paths["ig65m"] = os.path.join(
                    self.args.video_ig65m_path, _video_dir, str(_id).zfill(5) + ".npy"
                )
            if self.use_s3d_how100m:
                _video_paths["s3d"] = os.path.join(
                    self.args.video_s3d_path, _video_dir, str(_id).zfill(5) + ".npy"
                )
            self.videos.append(_video_paths)
            self._ids.append(str(_id).zfill(5))
        ...
        for _id in self.ids:
            ...
            if self.use_vit:
                _src_img_paths["vit"] = os.path.join(
                    self.args.img_extract_vit_path, _img_dir, str(_id).zfill(5) + ".npy"
                )
                _tgt_img_paths["vit"] = os.path.join(
                    self.args.img_tgt_vit_path, _img_dir, str(_id).zfill(5) + ".npy"
                )
            if self.use_effnet:
                _src_img_paths["effnet"] = os.path.join(
                    self.args.img_extract_eff_path, _img_dir, str(_id).zfill(5) + ".npy"
                )
                _tgt_img_paths["effnet"] = os.path.join(
                    self.args.img_tgt_eff_path, _img_dir, str(_id).zfill(5) + ".npy"
                )
    ...
    def _read_articles(self):
        ...
        df = pd.read_csv(
            os.path.join(path, f"{self.mode}_mms.tsv"),
            sep="\t",
            header=None,
